    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Foydalanuvchilarning xizmat haqidagi sharhlari</p>
        </div>
        <button onClick={load} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm font-medium transition-colors">
          🔄 Yangilash
        </button>
      </div>
      ...
      <div className="flex gap-3 mb-6">
        {[
          ...
          >
            {tab.emoji} {tab.label}
            <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${filter===tab.key?'bg-white/20 text-white':'bg-gray-100 text-gray-500'}`}>
              {counts[tab.key]}
            </span>
          </button>
        ))}
            ...
            <div key={r.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="flex items-start justify-between gap-4 mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-gray-900">{r.author_name || r.first_name || 'Foydalanuvchi'}</p>
              ...
              <div className="flex items-center gap-2 mb-3">
                <Stars n={r.stars} />
                <span className="text-sm font-semibold text-gray-600">{r.stars}/5</span>
              </div>

              {/* Text */}
              <blockquote className="text-sm text-gray-700 leading-relaxed bg-gray-50 rounded-xl p-3 border-l-4 border-indigo-200">
                "{r.text}"
              </blockquote>
              ...
              {r.status === 'pending' && (
                <div className="flex gap-2 mt-4">
                  ...
                  <button
                    ...
                    className="flex-1 py-2.5 rounded-xl bg-rose-100 hover:bg-rose-200 text-rose-700 text-sm font-bold transition-colors disabled:opacity-60"
                  >
              ...
              {r.status === 'published' && (
                <button
                  ...
                  className="mt-3 px-4 py-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 text-xs font-semibold transition-colors"
                >
              ...
              {r.status === 'rejected' && (
                <button
                  onClick={() => setStatus(r.id, 'published')}
                  disabled={!!actionLoading}
                  className="mt-3 px-4 py-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-600 text-xs font-semibold transition-colors"
                >
                  Nashr etish
                </button>
              )}
